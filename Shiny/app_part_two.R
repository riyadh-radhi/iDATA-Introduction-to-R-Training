...
ui <- dashboardPage(
  ...
  body = dashboardBody(
    fluidRow(
      ### second_plot ----
      box(
        selectInput(inputId = "talk_year_input",
                    label = "Select Talk Year",
                    choices = plt_df$talk_year,
                    selected = 2020),width = 3
      ),
      box(
        girafeOutput(outputId = "second_plot"),width = 12
      ),
      ### first_plot ----
      box(
...
## first_plot ----
  ...
  output$second_plot <-  renderGirafe({
    ggplot(data = filtered_data(), aes(x = talk_month, y = views)) +
      geom_point_interactive(aes(tooltip = title, onclick = link_action),
                             size = 3, 
                             color = "#2c5b78", alpha = 0.4) +
      theme_minimal() +
      ...
      theme(panel.grid.major.x = element_blank(),
            panel.grid.minor.x = element_blank(),
            plot.title = element_text(hjust = 0.5, color = "#3d3d3d"),
            axis.title = element_text(color = "#3d3d3d")) -> plot_two
